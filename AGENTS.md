...
- `just sdk-check` / `just python-sdk-check` — generated-contract drift, strict
  ...
  Those two catch a method that DRIFTED; the one that was never written is
  `check-sdk-coverage.sh` (in `lint-workflows`), which fails when a
  `domain::capability` entry has no method on a client — derived from the
  manifest and each client's own source, never a list. Its own red is
  behaviorally tested (`check-sdk-coverage-test.sh`), since a gate whose whole
  job is to fail is worthless unproven. `docs/sdk-parity.md` is the audit those
  gates hold in place, regenerated by `just parity-audit`.
