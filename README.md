...
mid-turn has no measured provider timing *because* it failed, and history keeps
it anyway with its timing absent rather than fabricated — a history that could
only hold successes would hide exactly the runs an operator has to see. Such a
record carries `status`, `exit_code`, the classified `failure_kind`, the measured
`duration_ms`, whatever partial transcript the failure left (`events`, `usage`,
any salvaged `text`), and `error`: the harness's own account of the failure as
...
produce. It arrived in history schema **v1.3**; a record's `schema_version` names
the oldest reader that can understand it, so a record carrying `error` declares
`1.3` while a provider-measured success still declares `1.1`.
