...
/// and — deliberately — a non-zero classified `rate_limit` (a transient
/// condition of a *working, authenticated* harness) or `model_not_found` (a
/// configuration mistake the user should see, not silently route around). A
/// [`Status::Planned`] dry-run row is not a run at all and is `None` too (the
/// fallback driver never executes under `--print-command`).
pub fn startup_failure_reason(status: Status, failure_kind: Option<&str>) -> Option<&'static str> {
    match status {
...
pub fn is_startup_failure(status: Status, failure_kind: Option<&str>) -> bool {
    startup_failure_reason(status, failure_kind).is_some()
}
...
mod tests {
    ...
    fn startup_failures_fall_through() {
        ...
        assert_eq!(
            startup_failure_reason(Status::Skipped, None),
            Some("not-installed")
        );
        assert_eq!(
            startup_failure_reason(Status::SpawnError, None),
            Some("spawn-error")
        );
        // Provisioning "could not run": rejected before any work.
        assert_eq!(
            startup_failure_reason(Status::Nonzero, Some("auth")),
            Some("auth")
        );
        assert_eq!(
            startup_failure_reason(Status::Nonzero, Some("quota")),
            Some("quota")
        );
        for status in [Status::Skipped, Status::SpawnError] {
            assert!(is_startup_failure(status, None));
        }
        assert!(is_startup_failure(Status::Nonzero, Some("auth")));
        assert!(is_startup_failure(Status::Nonzero, Some("quota")));
    }

    #[test]
    fn a_real_run_does_not_fall_through() {
        // A clean run, a slow run that timed out, and a dry-run row all stay.
        assert_eq!(startup_failure_reason(Status::Ok, None), None);
        assert_eq!(startup_failure_reason(Status::Timeout, None), None);
        assert_eq!(startup_failure_reason(Status::Planned, None), None);
        assert!(!is_startup_failure(Status::Ok, None));
        assert!(!is_startup_failure(Status::Timeout, None));
    }

    #[test]
    fn a_real_failure_does_not_fall_through() {
        // A plain non-zero task failure (no classified reason) is a real run.
        assert_eq!(startup_failure_reason(Status::Nonzero, None), None);
        assert!(!is_startup_failure(Status::Nonzero, None));
        // Transient / configuration reasons are NOT setup failures: falling
        // through a 429 would mask a working harness's real hiccup, and an
        // unknown model is a config mistake the user should see.
        for kind in ["rate_limit", "model_not_found", "something-else"] {
            assert_eq!(
                startup_failure_reason(Status::Nonzero, Some(kind)),
                None,
                "failure_kind {kind} must not fall through"
            );
            assert!(!is_startup_failure(Status::Nonzero, Some(kind)));
        }
    }
}
