...
with `fmt-check`, `lint`, `lint-doc`, `lint-sh`, `test-symlinked-tmp`, `coverage`, `build`, `smoke`.
...
| --- | --- | --- | --- | --- | --- |
| `run_report` | `batch`, `bypass_permissions`, `config_files`, `control`, `dry_run`, `fallback`, `fork`, `history_file`, `mock_rules`, `model`, `models`, `oneharness_version`, `permission_mode`, `prompt`, `results`, `resume`, `schema`, `schema_max_retries`, `schema_version`, `session`, `spy_file` | 86 | yes | yes | yes |
