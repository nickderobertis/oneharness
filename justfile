...
live-variants-tools:
    log="$(mktemp)"; if npm install -g @anthropic-ai/claude-code @openai/codex opencode-ai @qwen-code/qwen-code @charmland/crush >"$log" 2>&1; then rm -f "$log"; echo "live-variants-tools: installed Claude Code, Codex, OpenCode, Qwen Code, and Crush"; else cat "$log" >&2; rm -f "$log"; echo "live-variants-tools: npm install failed; resolve the reported error" >&2; exit 1; fi
