...
# llmlint: ignore[tool_output_is_signal] Like every other `live-*` recipe, this one
# forwards a live e2e transcript: the phase lines are what attribute a failure —
# or a hang inside a 15s freeze window — to a step in a log nobody can attach a
# debugger to. The script owns that contract (see its own file-level ignore).
