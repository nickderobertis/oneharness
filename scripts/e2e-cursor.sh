...
# (`--model 'MODEL[effort=high]'`), so it needs a real model to attach to — a
# forum report claims cursor-agent rejects the bracket syntax its own --help
# advertises, so this phase is the live proof either way (a rejection fails the
# run here, and oh_dump surfaces cursor's stderr — model-id vs. effort-option
# error). Uses a reasoning-specific model, scoped to this phase so it never
# disturbs the model-less phases above; note-and-continue (not skip()) when none
