...
# (Git Bash / MSYS / WSL). For unpublished targets, build from source with
# `cargo install --git https://github.com/nickderobertis/oneharness --locked`.
#
# Like the CLI it installs, this script never weakens silently: it aborts rather
# than install a binary it cannot checksum-verify.
...
Environment: ONEHARNESS_VERSION, ONEHARNESS_INSTALL_DIR, GITHUB_TOKEN.
EOF
}
...
detect_target() {
    ...
    if [ "$ext" = "zip" ] && [ "$arch_part" != "x86_64" ]; then
        err "no prebuilt Windows binary for $arch; install from source with 'cargo install --git https://github.com/$REPO --locked'"
    fi
...
# Download a release asset (follows redirects) to a file. Local paths are
# accepted for the hermetic e2e path that packages a just-built binary as a
# release-shaped archive.
download() {
    _url="$1"
    _out="$2"
    case "$_url" in
...
main() {
    ...
    needs_downloader=0
    [ -z "$version" ] && needs_downloader=1
    case "${ONEHARNESS_RELEASE_BASE_URL:-}" in
        "" | http://* | https://*) needs_downloader=1 ;;
    esac
    ...
    if have curl; then
        ...
        DL="wget"
    elif [ "$needs_downloader" -eq 1 ]; then
    ...
    base_url="${ONEHARNESS_RELEASE_BASE_URL:-https://github.com/$REPO/releases/download/${version}}"
    base_url="${base_url%/}"
    ...
    if [ ! -f "$src" ]; then
        # taiki-e/upload-rust-binary-action keeps the binary at the archive root,
        # but fall back to a search so a leading-dir layout still works.
